...
@Override
    ...
    {
        String sql = "UPDATE rendezvous SET nom = ? , prenom=? , cin = ? ,telephone = ? , jour = ? , date_heure = ? where cin = ? ";
        try {
            PreparedStatement pst = cnx.prepareStatement(sql);
            pst.setString(1, rendezvous.getNom());
            pst.setString(2, rendezvous.getPrenom());
            pst.setString(3, rendezvous.getCin());
            pst.setInt(4, rendezvous.getTelephone());
            pst.setString(5, rendezvous.getDate());
            pst.setString(6, rendezvous.getDate_heure());
            pst.executeUpdate();
        }catch (Exception e){System.out.print(e);}
